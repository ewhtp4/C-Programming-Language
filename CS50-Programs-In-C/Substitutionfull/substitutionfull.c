...
static string Decrypt(string cipherText, string key)  
{  
    char *decyphred = malloc(strlen(cipherText) * sizeof(*decyphred));;  
    for (int i = 0; i < strlen(cipherText); i++)  
    {  
        if (isupper(cipherText[i]))
        {  
            int index = key[cipherText[i]] + 65;  
            decyphred[i] = (putchar(toupper((char)index)));  
        }
        if (islower(cipherText[i]))
        {  
            int index = key[cipherText[i]] + 97;  
            decyphred[i] = (putchar(tolower((char)index)));  
        }
        if (!isalpha(cipherText[i])) 
        {
            decyphred[i] = cipherText[i];
        }
    }  
    return decyphred; 
}    